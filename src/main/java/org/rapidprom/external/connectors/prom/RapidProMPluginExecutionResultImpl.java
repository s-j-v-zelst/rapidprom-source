...
public class RapidProMPluginExecutionResultImpl extends PluginExecutionResultImpl {

	public RapidProMPluginExecutionResultImpl(Class<?>[] returnTypes, String[] returnNames, PluginDescriptor plugin) {
		...
		for (int i =0; i < objectArr.length; i++) {
			...
			if (!getType(i).equals(void.class)) {
				...
				if (!getType(i).isAssignableFrom(type)) {
					throw new IncorrectReturnTypeException(getPlugin().getName(), i, getType(i), objectArr[i].getClass());
				}
