...
public class GenerateProcessTreesOperator extends Operator {
	...
	protected final InputPort inputfile = getInputPorts().createPort("file", FileObject.class);
	protected final OutputPort outputPort = getOutputPorts().createPort("newick tree collection");

	public GenerateProcessTreesOperator(OperatorDescription description) {
		super(description);
		getTransformer().addRule(new GenerateNewMDRule(outputPort, IOObjectCollection.class));
