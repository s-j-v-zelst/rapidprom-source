...
public class TransitionSystemtoPetriNetConversionOperator extends Operator {
	...
	private InputPort input = getInputPorts().createPort("model (ProM Transition System)",
			TransitionSystemIOObject.class);
	...
	public void doWork() throws OperatorException {
		...
		logger.log(Level.INFO, "Start: transition system to petri net conversion");
		...
		PluginContext pluginContext = RapidProMGlobalContext.instance()
				.getFutureResultAwarePluginContext(TransitionSystem2Petrinet.class);

		Object[] result;
		try {
			result = converter.convertToPetrinet(pluginContext,
					input.getData(TransitionSystemIOObject.class).getArtifact());
		} catch (Exception e) {
			e.printStackTrace();
			throw new OperatorException("There was an error obtaining connected elements for this transition system");
		}

		PetriNetIOObject petriNet = new PetriNetIOObject((Petrinet) result[0], (Marking) result[1], null,
				input.getData(TransitionSystemIOObject.class).getPluginContext());

		output.deliver(petriNet);

		logger.log(Level.INFO, "End: transition system to petri net conversion ("
