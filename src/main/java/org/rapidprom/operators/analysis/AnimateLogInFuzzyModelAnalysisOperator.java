...
public class AnimateLogInFuzzyModelAnalysisOperator extends Operator {
	...
	private static final String PARAMETER_1_KEY = "Lookahead",
			PARAMETER_1_DESCR = "Indicates the size of the window of event that can be potentially connected to an event.",
	...
	private InputPort inputMetricsRepository = getInputPorts().createPort("model (MetricsRepository)",
	...
	public AnimateLogInFuzzyModelAnalysisOperator(OperatorDescription description) {
		super(description);
	...
	public void doWork() throws OperatorException {
		...
		PluginContext pluginContext = RapidProMGlobalContext.instance()
				.getFutureResultAwarePluginContext(FuzzyAdapterPlugin.class);
		MetricsRepositoryIOObject metricsRepository = inputMetricsRepository.getData(MetricsRepositoryIOObject.class);
		XLogIOObject xLog = inputXLog.getData(XLogIOObject.class);

		FuzzyAdapterPlugin adapter = new FuzzyAdapterPlugin();
		MutableFuzzyGraph fuzzyInstance = adapter.mineGeneric(pluginContext, metricsRepository.getArtifact());

		FuzzyAnimation animation = new FuzzyAnimation(pluginContext, fuzzyInstance, xLog.getArtifact(),
		...
		outputFuzzyAnimation.deliver(new FuzzyAnimationIOObject(animation, pluginContext));
		logger.log(Level.INFO,
				"End: animate event log in fuzzy model (" + (System.currentTimeMillis() - time) / 1000 + " sec)");
	}

	public List<ParameterType> getParameterTypes() {
		List<ParameterType> parameterTypes = super.getParameterTypes();

		ParameterTypeInt parameterType2 = new ParameterTypeInt(PARAMETER_1_KEY, PARAMETER_1_DESCR, 1, 25, 5, false);
		parameterTypes.add(parameterType2);

		ParameterTypeInt parameterType3 = new ParameterTypeInt(PARAMETER_2_KEY, PARAMETER_2_DESCR, 0, 15, 3, false);
