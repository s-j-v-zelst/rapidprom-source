...
public class MeasurePrecisionAnalysisOperator extends Operator {
	...
	private InputPort input = getInputPorts().createPort("alignments (ProM PNRepResult)", PNRepResultIOObject.class);
	...
	public void doWork() throws OperatorException {
		...
		logger.log(Level.INFO, "Start: measure precision/generalization based on alignments");
		long time = System.currentTimeMillis();

		PluginContext pluginContext = RapidProMGlobalContext.instance().getPluginContext();

		PNRepResultIOObject alignment = input.getData(PNRepResultIOObject.class);
		...
		if (alignment.getArtifact() != null) {
			...
			try {
				result = aligner.measureConformanceAssumingCorrectAlignment(pluginContext, alignment.getMapping(),
						alignment.getArtifact(), alignment.getPn().getArtifact(), alignment.getPn().getInitialMarking(),
						getParameterAsBoolean(PARAMETER_1));

				fillTableWithRow(table, "Precision", result.getPrecision(), attributes);
				fillTableWithRow(table, "Generalization", result.getGeneralization(), attributes);
		...
		logger.log(Level.INFO, "End: measure precision/generalization based on alignments ("
	...
	public List<ParameterType> getParameterTypes() {
		...
		ParameterTypeBoolean parameterType1 = new ParameterTypeBoolean(PARAMETER_1, PARAMETER_1, true);
