...
public class MergeTwoEventLogsOperator extends Operator {
	...
	private InputPort inputLog1 = getInputPorts().createPort("event log 1 (ProM Event Log)", XLogIOObject.class);
	private InputPort inputLog2 = getInputPorts().createPort("event log 2 (ProM Event Log)", XLogIOObject.class);
	...
	public void doWork() throws OperatorException {
		...
		XLog result = XFactoryRegistry.instance().currentDefault().createLog(xLog1.getAttributes());
		...
		// report the result
		XLogIOObject xLogIOObject = new XLogIOObject(result, logIO1.getPluginContext());
		...
		logger.log(Level.INFO, "End: merge event logs (" + (System.currentTimeMillis() - time) / 1000 + " sec)");
	...
	public List<ParameterType> getParameterTypes() {
		...
		ParameterTypeBoolean parameterType1 = new ParameterTypeBoolean(PARAMETER_1_KEY, PARAMETER_1_DESCR, false);
