...
public class SplitXLogOperator extends Operator {
	...
	private final InputPort inputXLog = getInputPorts().createPort("event log", XLogIOObject.class);

	private OutputPort outputLog1 = getOutputPorts().createPort("event log 1");

	private OutputPort outputLog2 = getOutputPorts().createPort("event log 2");

	public SplitXLogOperator(OperatorDescription description) {
		super(description);
		getTransformer().addRule(new GenerateNewMDRule(outputLog1, XLogIOObject.class));
		getTransformer().addRule(new GenerateNewMDRule(outputLog2, XLogIOObject.class));
	}
	...
	public void doWork() throws OperatorException {
		...
		for (int i = 0; i < original.size(); i++) {
			XTrace newTrace = factory.createTrace(original.get(i).getAttributes());
	...
	public List<ParameterType> getParameterTypes() {
		...
		ParameterType type = new ParameterTypeDouble(PARAMETER_1_KEY, PARAMETER_1_KEY, 0, 1, 0.5, false);
		types.add(type);
		return types;
	}
