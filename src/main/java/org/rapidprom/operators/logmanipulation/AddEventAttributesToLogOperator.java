...
public class AddEventAttributesToLogOperator extends Operator {
	...
	public void doWork() throws OperatorException {
		...
		if (traceIdColumnAttrib != null && eventIdColumnAttrib != null) {
			...
			// show warning
			JOptionPane.showMessageDialog(null, "Case ID column or event ID column was not found",
					"Case ID / Event ID column not found", JOptionPane.ERROR_MESSAGE);
			outputLog.deliverMD(md);
			outputLog.deliver(null);
		}
		logger.log(Level.INFO, "End: add event attributes (" + (System.currentTimeMillis() - time) / 1000 + " sec)");
	}
	...
	private XLog mergeExampleSetIntoLog(XLog xLog, ExampleSet es, Attribute traceIdColumnAttrib,
			Attribute eventIdColumnAttrib) {
