...
public class AddEventsToLogOperator extends Operator {
	...
	private static final String PARAMETER_1 = "Case id column", PARAMETER_2 = "Event id column",
			PARAMETER_3 = "Lifecycle column", PARAMETER_4 = "Timestamp column", PARAMETER_5 = "Resource column";
	...
	public void doWork() throws OperatorException {
		...
		if (traceIdColumnAttrib != null) {
			...
			dumpSizeTraces(xLog);
			XLog adaptedLog = mergeExampleSetIntoLog(xLog, es, traceIdColumnAttrib);
		...
		logger.log(Level.INFO, "End: add event (" + (System.currentTimeMillis() - time) / 1000 + " sec)");
	}
	...
	private XLog mergeExampleSetIntoLog(XLog xLog, ExampleSet es, Attribute traceIdAttrib)
			...
			if (t != null) {
				...
				while (iterator2.hasNext()) {
					...
					if (nameAttrib.equals(traceIdAttrib.getName())) {
						// do nothing
					} else if (nameAttrib.equals(getParameterAsString(PARAMETER_2))
							&& !getParameterAsString(PARAMETER_2).equals("")) {
						// concept:name
						String value = row.getValueAsString(next);
						XAttributeLiteral attribNameEvent = factory.createAttributeLiteral("concept:name", value,
								XConceptExtension.instance());
						attribMapEvent.put("concept:name", attribNameEvent);
					} else if (nameAttrib.equals(getParameterAsString(PARAMETER_3))
							&& !getParameterAsString(PARAMETER_3).equals("")) {
						// lifecycle:transition
						String value = row.getValueAsString(next);
						XAttributeLiteral attribLC = factory.createAttributeLiteral("lifecycle:transition", value,
								XLifecycleExtension.instance());
						attribMapEvent.put("lifecycle:transition", attribLC);
					} else if (nameAttrib.equals(getParameterAsString(PARAMETER_4))
							&& !getParameterAsString(PARAMETER_4).equals("")) {
						// timestamp
						Date dateValue = row.getDateValue(next);
						XAttributeTimestamp attribTimestampEvent = factory.createAttributeTimestamp("time:timestamp",
								dateValue, XTimeExtension.instance());
						attribMapEvent.put("time:timestamp", attribTimestampEvent);
					} else if (nameAttrib.equals(getParameterAsString(PARAMETER_5))
							&& !getParameterAsString(PARAMETER_5).equals("")) {
						// resource
						String value = row.getValueAsString(next);
						XAttributeLiteral attribResource = factory.createAttributeLiteral("org:resource", value,
								XOrganizationalExtension.instance());
						attribMapEvent.put("org:resource", attribResource);
					} else {
						if (next.getValueType() == Ontology.DATE || next.getValueType() == Ontology.DATE_TIME) {
							Date dateValue = row.getDateValue(next);
							XAttributeTimestamp attribTimestampEvent = factory.createAttributeTimestamp(nameAttrib,
									dateValue, XTimeExtension.instance());
							attribMapEvent.put(nameAttrib, attribTimestampEvent);
						} else {
							String value = row.getValueAsString(next);
							XAttributeLiteralImpl attribLit = new XAttributeLiteralImpl(nameAttrib, value);
	...
	public List<ParameterType> getParameterTypes() {
		...
		ParameterTypeString parameterType2 = new ParameterTypeString(PARAMETER_2, PARAMETER_2, "E:concept:name");
		parameterTypes.add(parameterType2);

		ParameterTypeString parameterType3 = new ParameterTypeString(PARAMETER_3, PARAMETER_3,
				"E:lifecycle:transition");
		parameterTypes.add(parameterType3);

		ParameterTypeString parameterType4 = new ParameterTypeString(PARAMETER_4, PARAMETER_4, "E:time:timestamp");
		parameterTypes.add(parameterType4);

		ParameterTypeString parameterType5 = new ParameterTypeString(PARAMETER_5, PARAMETER_5, "E:org:resource");
