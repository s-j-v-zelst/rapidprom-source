...
public class ImportXSStaticXSEventStreamOperator
	...
	private final static String[] SUPPORTED_FILE_FORMATS = new String[] { "evst" };

	public ImportXSStaticXSEventStreamOperator(OperatorDescription description) {
		super(description, XSStaticXSEventStreamIOObject.class, SUPPORTED_FILE_FORMATS);
	}
	...
	protected XSStaticXSEventStreamIOObject read(File file) throws Exception {
		...
		PluginContext context = RapidProMGlobalContext.instance()
				.getFutureResultAwarePluginContext(XSStaticXSEventStreamImportPlugin.class);
		XSStaticXSEventStream staticStream = (XSStaticXSEventStream) importer.importFile(context, file);
