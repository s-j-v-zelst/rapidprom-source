...
public class ImportCPNModelOperator extends AbstractRapidProMImportOperator<CPNModelIOObject> {

	private final static String[] SUPPORTED_FILE_FORMATS = new String[] { "cpn" };
	...
	protected CPNModelIOObject read(File file) throws Exception {
		...
		PluginContext context = RapidProMGlobalContext.instance()
				.getFutureResultAwarePluginContext(LoadCPNModelFromFile.class);
		net = LoadCPNModelFromFile.importColouredPetriNetFromStream(context, new FileInputStream(file), file.getName(),
				file.length());
